...
def get_predicted_features(df, df_name, col_name):
    ...
    for key, value in df.items():
        if len(value) == 0:
            continue
        r = list(range(len(value)))
        random.shuffle(r)
        
        for i in r:
            structures.append(Structure.from_str(df[key][col_name[key[0]]+'_structure'][i], fmt='cif'))
    
    s_exp = []
    data_path = '../data/data_folder/data/all_data.json' # put here the path to the json file
    with open(data_path,'r') as fp:
        d = json.load(fp)
        s_exp = [Structure.from_dict(x['structure']) for x in d['ordered_exp'].values()]
        fp.close()
    
    r = list(range(len(s_exp)))
    random.shuffle(r)
    for i in r:
    ...
    pca_predicted_features = np.array(predicted_features)
...
def megnet_pca(df_total, df_name,  col_name):
    # pca_total = {}
    # for name in df_total.keys():
    pca_total = get_predicted_features(df_total, df_name, col_name)
...
for key, value in col_name.items():
    df_total[key] = pd.read_csv('../data/data_folder/data/{0}_cif.csv'.format(value))
...
for l in df_name:
    for name in l:
        pca_loc[name] = [s, s + len(df_total[name])]
        print(name, ':', pca_loc[name])
        s += len(df_total[name])
...
model_formation_energy = MEGNetModel.from_file('../data/data_folder/band_gap_regression.hdf5')
...
pca_total = megnet_pca(df_total, df_name, col_name)
pca_each = {}
for l in df_name:
    for name in l:
        loc = pca_loc[name]
        pca_each[name] = pca_total[loc[0]:loc[1]]
        print(name, ':', len(pca_each[name]))
pca_each['E'] = pca_total[pca_loc['PSHG'][1]:]
print('E', ':', len(pca_each['E']))
