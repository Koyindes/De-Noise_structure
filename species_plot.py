...
el_lst_lst = []
el_lst_lst.append(el_lst[:int(len(el_lst)/4)])
el_lst_lst.append(el_lst[int(len(el_lst)/4):int(len(el_lst)/4)*2])
el_lst_lst.append(el_lst[int(len(el_lst)/4)*2:int(len(el_lst)/4)*3])
el_lst_lst.append(el_lst[int(len(el_lst)/4)*3:])
'''
el_lst_lst.append(el_lst[:int(len(el_lst)/3)])
el_lst_lst.append(el_lst[int(len(el_lst)/3)+1:int(len(el_lst)/3)*2])
el_lst_lst.append(el_lst[int(len(el_lst)/3)*2+1:])
'''
...
for j in range(2):
    ...
    for i in range(2):
        x = np.arange(len(el_lst_lst[j*2+i]))  # the label locations
        ax = axx[i]
        # Add some text for labels, title and custom x-axis tick labels, etc.
        # ax.set_ylabel('Scores')
        # ax.set_title('Scores by group and gender')
        ax.set_xticks(x)
        ax.set_xticklabels(el_lst_lst[j*2+i], rotation = 75, fontsize=20)# , ha="right")
        ax.set_ylim(0, 1.7)
        ...
        scan_y = []
        for e in el_lst_lst[j*2+i]:
            if e in scan_dict.keys():
                scan_y.append(scan_dict[e])
            else:
                scan_y.append(0)
        ...
        gllb_y = []
        for e in el_lst_lst[j*2+i]:
            if e in gllb_dict.keys():
                gllb_y.append(gllb_dict[e])
            else:
                gllb_y.append(0)
    
        max_array = np.maximum(np.maximum(np.log(np.array(pbe_y)), np.log(np.array(hse_y))), np.log(np.array(scan_y)), np.log(np.array(gllb_y)), np.log(np.array(exp_y)))
    
        ax.bar(x, np.log(np.array(pbe_y))/max_array, width, label='PBE')
        ax.bar(x + width, np.log(np.array(hse_y))/max_array, width, label='HSE')
        ax.bar(x - width, np.log(np.array(scan_y))/max_array, width, label='SCAN')
        ax.bar(x + 2*width, np.log(np.array(gllb_y))/max_array, width, label='GLLB')
        ax.bar(x - 2*width, np.log(np.array(exp_y))/max_array, width, label='EXP')
        
        for i in range(len(pbe_y)):
            ax.text(i-0.2, 1.05, pbe_y[i], rotation = 75, fontsize=16)
        
        ax.tick_params(labelsize=20)
    
    if j == 0:
        axx[0].legend(loc = 0, ncol = 3, fontsize = 14)
    # plt.tick_params(labelsize=13)
    fig.tight_layout()
    plt.subplots_adjust(top=0.994,bottom=0.079,left=0.038,right=0.995,hspace=0.234,wspace=0.195)
    plt.show()
